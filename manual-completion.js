...
function typeOf(card){
  const s=(card.querySelector('.eyebrow')?.textContent||'').toLowerCase();
  return s.startsWith('match')?'match':s.startsWith('entraînement')||s.startsWith('entrainement')?'training':'event';
}
...
async function sync(){
  ...
  const list=await events(), map=new Map(list.map(e=>[e.id,e]));
  ...
  host.querySelectorAll('.life-card[data-detail]').forEach(card=>{
    ...
    if(!history&&!e.cancelled){
      ...
      if(!actions.querySelector('[data-finish]')){
        const b=document.createElement('button');b.type='button';b.className='btn primary small-btn';b.dataset.finish=e.id;b.textContent='✓ Terminer';b.onclick=ev=>{ev.stopPropagation();finish(e.id,e.title)};actions.prepend(b);
      }
    }
  });
  ...
  if(history){
    ...
    list.filter(e=>e.completed&&!e.cancelled).sort((a,b)=>new Date(b.completed_at||b.event_date)-new Date(a.completed_at||a.event_date)).forEach(e=>{
      const card=document.createElement('div');card.className='card life-card';card.dataset.manualCompleted=e.id;
      card.dataset.eventType=e.type;
